...
export default function SalesUpcomingOrders() {
  ...
  const loadData = async () => {
    ...
    setError(null);
    try {
      const res = await fetchWithAuth(API.DASHBOARD.SALES_UPCOMING_ORDERS);
      if (res.ok) {
  ...
  if (loading) {
    return (
      <div className="w-full">
        <div className="rounded-2xl bg-white dark:bg-[#1F2933] shadow-lg p-6 min-h-[455px] border border-slate-100 dark:border-[#4B5563]">
      ...
      <div className="rounded-xl bg-white dark:bg-[#1F2933] shadow-sm p-6 min-h-[500px] border border-[#E5E7EB] dark:border-[#4B5563] hover:shadow-md transition-all duration-300">
        ...
        {error && (
          <div className="mt-8 text-center py-12 rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-800">
            ...
            <p className="text-sm text-red-500 dark:text-red-300 mb-6 max-w-sm mx-auto">{error}</p>
            <button
              onClick={loadData}
          ...
          <div className="space-y-2">
            {stats.map((s, idx) => (
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  {idx === 0 && <div className="w-2 h-2 bg-[#3B82F6] rounded-full animate-pulse" />}
                  <span
        ...
        {!error && (!stats || stats.length === 0) && (
          <div className="mt-8 text-center py-12 rounded-xl bg-[#F7F7F7] dark:bg-[#2C3540] border border-[#E5E7EB] dark:border-[#4B5563]">
            ...
            <p className="text-sm text-[#4B5563] dark:text-[#9CA3AF] mb-4">
              No upcoming orders found for the next 5 days
            </p>
          </div>
        )}
      </div>
