...
import { useReport, ReportRow, TimelineNode } from "@/app/admin/components/hooks/useReport";

function StageStepper({ timeline = [] }: { timeline: TimelineNode[] }) {
  const theme = useTheme();
  const isDark = theme.palette.mode === "dark";

  const GREEN = "#22c55e";
  const YELLOW = "#FFD200"; // Fanuc Yellow for pending
      ...
      {timeline.map((step, idx) => {
        ...
        if (isDone) {
          ...
          circleSx = {
            ...
            bgcolor: YELLOW,
            borderColor: YELLOW,
            color: "#111", // Dark icon on yellow background
            boxShadow: `0 0 8px ${YELLOW}55`,
          };
        } else {
          circleSx = {
            ...circleSx,
        ...
        const labelColor = isDone
          ...
          : isPending
            ? "#ca8a04"
            : dimText;
        const labelWeight = isDone || isPending ? 600 : 400;

        const nextStep = !isLast ? timeline[idx + 1] : null;
        const nextDone = nextStep?.state === "completed";
        const lineColor =
          isDone && nextDone ? GREEN : isDone ? `${GREEN}44` : dimLine;
        ...
        return (
          ...
          >
            ...
            >
              <Box sx={circleSx}>
                {isDone && <CheckIcon sx={{ fontSize: 11, color: "#fff" }} />}
                {isPending && (
                  <HourglassEmptyIcon sx={{ fontSize: 11, color: "#111" }} />
                )}
                {!isDone && !isPending && (
                  <CheckIcon sx={{ fontSize: 11, color: dimColor }} />
                )}
              </Box>
                      ...
                      <React.Fragment key={group.customerName}>
                        ...
                        {group.rows.map((row) => (
                          ...
                          >
                            ...
                            >
                              <StageStepper timeline={row.statusHubTimeline} />
                            </TableCell>
                          </TableRow>
                        ))}
                      </React.Fragment>
