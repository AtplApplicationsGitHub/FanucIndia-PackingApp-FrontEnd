...
import {
  ...
  TableRow, TablePagination, Paper, alpha, useTheme,
} from "@mui/material";
        ...
        <TableContainer component={Paper} elevation={0}>
          <Table
            sx={{
              ...
              "& .MuiTableCell-root": { borderBottom: "none", py: 0.5, px: 1, fontSize: "0.875rem", whiteSpace: "nowrap" },
            }}
          >
            <TableHead sx={{ bgcolor: theme.palette.mode === "dark" ? "#000" : "#fff" }}>
              <TableRow sx={{ height: 60 }}>
                {["PRODUCT", "SALE ORDER NUMBER", "OUT BOUND DELIVERY", "TRANSFER ORDER", "REQUIRED DATE", "PAYMENT", "SALES ZONE", "CUSTOMER", "STATUS", "ARCHIVED AT"].map((head) => (
                  <TableCell key={head} sx={{ fontWeight: 700, color: theme.palette.mode === "dark" ? "#fff" : "#000" }}>
                    {head}
                  </TableCell>
                ))}
              </TableRow>
                ...
                <TableRow key={row.id}>
                  ...
                  <TableCell>
                    <MuiLink component={Link}
                      ...
                      underline="hover" sx={{ fontWeight: 500 }}>
                      {row.saleOrderNumber}
                    </MuiLink>
                  </TableCell>
                  <TableCell>{row.outboundDelivery ?? "-"}</TableCell>
                  <TableCell>{row.transferOrder ?? "-"}</TableCell>
                  <TableCell>{row.requiredDate ? formatDate(row.requiredDate) : "-"}</TableCell>
                  <TableCell>
                    <Box sx={{
                      display: "inline-flex", alignItems: "center", justifyContent: "center",
                      padding: "3px 10px", borderRadius: "16px", border: "1px solid", minWidth: "50px",
                  ...
                  <TableCell>
                    {(() => {
                      ...
                      const colorMap: Record<string, string> = { R105: "#3b82f6", W105: "#eab308", F105: "#8b5cf6", Dispatched: "#10b981" };
                      const colorMain = colorMap[row.status] ?? theme.palette.grey[500];
                      return (
                        <Box sx={{
                          display: "inline-flex", alignItems: "center", justifyContent: "center",
