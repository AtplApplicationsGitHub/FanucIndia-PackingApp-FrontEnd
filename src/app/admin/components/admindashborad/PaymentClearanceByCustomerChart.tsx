...
export default function PaymentClearanceByCustomerChart({ selectedDate, displayDate }: Props) {
  const { data, loading, error, refetch } = usePaymentClearanceByCustomer(selectedDate);
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 mb-6">
        ...
        <button
          ...
          className="flex items-center gap-2.5 px-4 py-2.5 bg-[#F7F7F7] dark:bg-[#2C3540] hover:bg-gray-200 dark:hover:bg-gray-600 
            text-[#1F2933] dark:text-[#F7F7F7] font-medium rounded-lg transition-all duration-200 border border-[#E5E7EB] dark:border-[#4B5563]"
        >
          {viewMode === "chart" ? (
            <>
      ...
      {viewMode === "chart" ? (
        <div className="h-[450px] -mx-6 -mb-6 text-gray-700 dark:text-gray-200">
          <BarChart
            ...
            series={[
              { dataKey: "cleared", label: "Payment Cleared", color: COLORS.cleared },
              { dataKey: "pending", label: "Payment Pending", color: COLORS.pending },
            ]}
          ...
          <table className="w-full text-sm border-t border-[#E5E7EB] dark:border-[#4B5563]">
            <thead className="bg-[#F7F7F7] dark:bg-[#2C3540]">
              <tr>
                ...
                <th className="px-6 py-4 text-center font-semibold" style={{ color: COLORS.cleared }}>Payment Cleared</th>
                <th className="px-6 py-4 text-center font-semibold" style={{ color: COLORS.pending }}>Payment Pending</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#E5E7EB] dark:divide-[#4B5563]">
              {paginatedData.length === 0 ? (
                 <tr><td colSpan={3} className="px-6 py-8 text-center text-gray-500">No data available</td></tr>
              ) : (
                paginatedData.map((row) => (
                  <tr key={row.customer} className="hover:bg-[#F7F7F7] dark:hover:bg-[#2C3540] transition bg-white dark:bg-[#1F2933]">
                    ...
                    <td className="px-6 py-4 text-center font-bold" style={{ color: COLORS.cleared }}>{row.cleared}</td>
                    <td className="px-6 py-4 text-center font-bold" style={{ color: COLORS.pending }}>{row.pending}</td>
                  </tr>
                ))
              )}
            </tbody>
