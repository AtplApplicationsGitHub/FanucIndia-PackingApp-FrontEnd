...
import { Database, Menu as MenuIcon, X } from "lucide-react";
import { useTheme } from "@mui/material";
import Image from "next/image";
import UserMenu from "@/common/components/UserMenu";
import packageJson from "../../../../package.json";

export type SuperAdminDashboardView = "master";

type Props = {
  userName: string;
  view: SuperAdminDashboardView;
  setView: (view: SuperAdminDashboardView) => void;
};
...
export default function SuperAdminDashboardHeader({ userName, view, setView }: Props) {
  const theme = useTheme();
  const [drawerOpen, setDrawerOpen] = useState(false);

  const handleMenuItemClick = (value: string) => {
    setView(value as SuperAdminDashboardView);
    setDrawerOpen(false);
  };
      ...
      >
        ...
        >
              ...
              {menuItems.map((item) => {
                ...
                return (
                  <Button
                    ...
                    disableRipple
                    variant="text"
                    onClick={() => setView(item.value as SuperAdminDashboardView)}
                    sx={{
                      borderRadius: 0,
                      px: 1.5,
                      py: 1,
          ...
          <Box sx={{ ml: { xs: 0.5, md: 0 }, display: "flex", alignItems: "center", gap: { xs: 0.5, md: 1 } }}>
            <UserMenu username={userName} userRole="SUPER_ADMIN" variant="full" />
            <IconButton
